...
# Function to create symptom-disease mapping with top 10 probabilities
def create_top10_symptom_disease_mapping(train_data, test_data, validate_data):
    ...
    for disease in all_data['PATHOLOGY'].unique():
        ...
        for symptom in disease_data['EVIDENCES'].unique():
            ...
            # Store the probability for the symptom
            symptom_probabilities[symptom] = prob

        # Sort symptoms by probability and select the top 10
        top_symptoms = dict(sorted(symptom_probabilities.items(), key=lambda x: x[1], reverse=True)[:10])

        # Add the disease and its associated top 10 symptom probabilities to the mapping
        symptom_disease_mapping[disease] = top_symptoms

    return symptom_disease_mapping

# Create the symptom-disease mapping with probabilities
print("Creating top 10 symptom-disease mapping...")
top10_symptom_disease_mapping = create_top10_symptom_disease_mapping(train_data, test_data, validate_data)

# Save the mapping to a JSON file
output_mapping_file = os.path.join(processed_data_folder, "top10_symptom_disease_mapping.json")
with open(output_mapping_file, 'w') as f:
    json.dump(top10_symptom_disease_mapping, f, indent=4)

print(f"Top 10 symptom-disease mapping saved to: {output_mapping_file}")
